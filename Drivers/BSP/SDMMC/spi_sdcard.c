...
static void sd_spi_init(void)
{
    ...
    sys_gpio_set(SD_CS_GPIO_PORT, SD_CS_GPIO_PIN,
                 SYS_GPIO_MODE_OUT, SYS_GPIO_OTYPE_PP, SYS_GPIO_SPEED_HIGH, SYS_GPIO_PUPD_PU);  /* SD CS 引脚模式设置(推挽输出) */

    /* 设置硬件上与SD卡相关联的控制引脚输出 */
    /* 禁止其他外设(NRF/25Q64)对SD卡产生影响 */
    RCC->APB2ENR |= 1 << 2;     /* PORTA时钟使能 */
    GPIOA->CRL &= 0XFFF0F0FF;
    GPIOA->CRL |= 0X00030300;   /* PA2.4 推挽 */
    GPIOA->ODR |= 0X5 << 2;     /* PA2.4 输出1 */
