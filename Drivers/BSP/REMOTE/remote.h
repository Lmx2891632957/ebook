...
#define REMOTE_IN_GPIO_PIN                      SYS_GPIO_PIN1
#define REMOTE_IN_GPIO_CLK_ENABLE()             do{ RCC->APB2ENR |= 1 << 2; }while(0)   /* PA口时钟使能 */


#define REMOTE_IN_TIMX                          TIM5
#define REMOTE_IN_TIMX_IRQn                     TIM5_IRQn
#define REMOTE_IN_TIMX_IRQHandler               TIM5_IRQHandler
#define REMOTE_IN_TIMX_CHY                      2                           /* 通道Y,  1<= Y <=4*/ 
#define REMOTE_IN_TIMX_CCRY                     REMOTE_IN_TIMX->CCR2
#define REMOTE_IN_TIMX_CHY_CLK_ENABLE()         do{ RCC->APB1ENR |= 1 << 3; }while(0)   /* TIMX 时钟使能 */
