...
int8_t STORAGE_GetCapacity (uint8_t lun, uint32_t *block_num, uint16_t *block_size)
{
    switch (lun)
    {
        ...
        case 1: /* SD卡 */
            *block_size = 512;
            *block_num = sd_get_sector_count();     /* 获取SD卡扇区数 */
            break;
    }
...
int8_t STORAGE_Read (uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    ...
    switch (lun)
    {
        ...
        case 1: /* SD卡 */
            ...
            while (res && retry)   /* 读出错, 尝测重读 */
            {
                retry--;
                if (res != 1)sd_init(); /* 重新初始化SD卡 */
                res = sd_read_disk(buf, blk_addr, blk_len);
                //printf("usb sd rd err:%d\r\n", res);
            }
...
int8_t STORAGE_Write (uint8_t lun, uint8_t *buf, uint32_t blk_addr, uint16_t blk_len)
{
    ...
    switch (lun)
    {
        ...
        case 1: /* SD卡 */
            ...
            while (res && retry)   /* 写出错, 尝测重写 */
            {
                retry--;
                if (res != 1)sd_init(); /* 重新初始化SD卡 */
                res = sd_write_disk(buf, blk_addr, blk_len);
                //printf("usb sd wr err:%d\r\n", res);
            }
