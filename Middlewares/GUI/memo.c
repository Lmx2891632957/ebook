...
uint8_t memo_check(_memo_obj *memox, void *in_key)
{
    ...
    switch (key->intype)
    {
        case IN_TYPE_TOUCH:     /* touch: click to position cursor */
            memo_cursor_flash(memox);

            if (memox->top < key->y && key->y < (memox->top + memox->height) && (memox->left < key->x) && key->x < (memox->left + memox->width))
            {
                memox->sta |= 1 << 7;
            ...
            else if (memox->sta & (1 << 7))
            {
                if ((memox->sta & 0x60) == (1 << 6))
                {
                    for (i = 0; i < memox->scbv->itemsperpage; i++)
                    {
                        if (memox->typos <= (memox->top + (i + 1)*memox->font) && memox->typos >= (memox->top + i * memox->font))break;
                    }

                    if (memox->scbv->topitem + i < memox->scbv->totalitems)
                    {
                        memo_show_cursor(memox, 0);
                        memox->lin = memox->scbv->topitem + i;
        ...
        case IN_TYPE_KEY:   /* physical keys: page up/down */
            if (key->keyval == 2)   /* KEY1_PRES: next page */
            {
                if (memox->scbv->totalitems > memox->scbv->itemsperpage)
                {
                    if (memox->scbv->topitem + memox->scbv->itemsperpage < memox->scbv->totalitems)
                    {
                        memox->scbv->topitem += memox->scbv->itemsperpage;
                    }
                    else
                    {
            ...
            else if (key->keyval == 3)   /* WKUP_PRES: previous page */
            {
                if (memox->scbv->topitem >= memox->scbv->itemsperpage)
                {
                    memox->scbv->topitem -= memox->scbv->itemsperpage;
                }
                else
...
void memo_cursor_set(_memo_obj *memox, uint16_t x)
{
    ...
    strlen = memox->width / (memox->font / 2);
...
void memo_show_cursor(_memo_obj *memox, uint8_t sta)
{
    ...
    if (sta == 0)   /* 不要显示光标 */
    {
        ...
        if (memo_get_cursorxpos(memox) + y <= memox->left + memox->width)   /* 只有不超过最左边的长度的时候,才允许显示 */
        {
            y = memo_get_cursorypos(memox); /* 得到y坐标 */
            gui_phy.back_color = memox->textbkcolor; /* 设置背景色 */

            if (y < memox->top + memox->height)gui_show_ptstr(memo_get_cursorxpos(memox), y, memox->left + memox->width, memox->top + memox->height - 1, 0, memox->textcolor, memox->font, txt, 0);
        }
    }
...
{
    gui_fill_rectangle(memox->left, memox->top, memox->width, memox->height, memox->textbkcolor); /* 填充内部 */
    gui_show_string(memox->text + memox->offsettbl[memox->scbv->topitem], memox->left, memox->top, memox->width, memox->height, memox->font, memox->textcolor);
...
uint8_t memo_info_update(_memo_obj *memox, uint32_t curpos)
{
    ...
    uint16_t linelenth = memox->width; /* 每行长度 */
...
void memo_draw_memo(_memo_obj *memox, uint8_t mode)
{
    ...
    gui_fill_rectangle(memox->left, memox->top, memox->width, memox->height, MEMO_DFT_TBKC); /* 填充内部 */
